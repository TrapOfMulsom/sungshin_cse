...
static unsigned long crc_table[128];

void gen_crc_table()//CRC 테이블 생성
{
    register int i, j;
...
void main()
{
    ...
    for (int i = 0; i < 32; i++) { // 32번 반복
        random = rand() % 9; // 난수 생성 (1초에 보통 최대 걸음 수를 9 라고 설정)
        InputText[i] = random; // plaintext 에 랜덤 값 넣어서 랜덤 걸음 수 출력
    }
    ...
    //초기화 벡터 선언
    BYTE pbszIV[4] = { 0x00,0x01,0x01,0x00 };
    in_frame[0] = pbszIV[0];
    in_frame[1] = pbszIV[1];
    in_frame[2] = pbszIV[2];
    in_frame[3] = pbszIV[3];
...
#define process_blockLeng   32
    ...
    //printf("\nCRC 코드 = %lX", crc); //송신측에서 생성한 CRC 코드
    ...
    printf("\n에러 검출중... = %lX\n", check);//CRC 코드 검증
    if (check != 0) {//손상이 이루어졌다면 복호화 과정을 진행하지 않음
        printf("에러 발생! 복호화 실패....");
        return 0;
    }
